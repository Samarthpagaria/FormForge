...
const getHeatmapColor = (level: number) => {
  switch(level) {
    case 4: return "bg-[#2d351e] dark:bg-[#d9e5c9]"; 
    case 3: return "bg-[#4a5832] dark:bg-[#aabf86]"; 
    case 2: return "bg-[#8ba059] dark:bg-[#728549]"; 
    case 1: return "bg-[#d9e5c9] dark:bg-[#2d351e]"; 
    default: return "bg-neutral-100 dark:bg-zinc-800/50"; 
  }
};
...
export default function AnalyticsPage() {
  ...
  return (
    <div className="relative z-0 min-h-[calc(100vh-64px-1rem)] bg-[#f5f5f3] dark:bg-zinc-950 flex flex-col p-4 md:px-6 m-2 rounded-2xl border border-neutral-200/60 dark:border-zinc-800/60 shadow-sm overflow-x-hidden overflow-y-auto">
      ...
      <div className="flex-1 w-[90%] mx-auto relative z-10 flex flex-col pb-12">
        ...
        <div className="flex flex-col xl:flex-row xl:items-start justify-between gap-6 mb-8 mt-4">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-neutral-800 dark:text-zinc-100 tracking-tight">Global Analytics</h1>
              <p className="text-neutral-500 dark:text-zinc-400 text-sm mt-0.5">Overview of all forms</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 mb-6">
          {/* Date Range */}
          <div className="flex items-center gap-4 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md p-3 rounded-xl border border-neutral-200/60 dark:border-zinc-800/60 shadow-sm self-start">
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-2 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md px-4 py-3 h-[48px] rounded-xl border border-neutral-200/60 dark:border-zinc-800/60 shadow-sm text-sm font-semibold text-neutral-700 dark:text-zinc-300 hover:bg-white dark:hover:bg-zinc-800 transition-colors"
            >
            ...
            <AnimatePresence>
              {isDropdownOpen && (
                <motion.div
                  ...
                  className="absolute top-full left-0 mt-2 w-64 bg-white dark:bg-zinc-900 border border-neutral-200/60 dark:border-zinc-800/60 rounded-xl shadow-xl z-50 p-2 max-h-64 overflow-y-auto"
                >
                  <div className="flex items-center justify-between px-3 py-2 border-b border-neutral-100 dark:border-zinc-800 mb-2">
                    <span className="text-xs font-bold text-neutral-500 dark:text-zinc-400 uppercase tracking-wider">Filter Forms</span>
                    {selectedFormIds.length > 0 && (
                      <button 
                        onClick={() => setSelectedFormIds([])}
                        className="text-xs font-semibold text-violet-600 dark:text-violet-400 hover:text-violet-700 dark:hover:text-violet-300"
                      >
                        Clear
                      </button>
                    )}
                  ...
                  {allForms?.map(form => (
                    <label key={form.id} className="flex items-center gap-3 px-3 py-2 hover:bg-neutral-50 dark:hover:bg-zinc-800/50 rounded-lg cursor-pointer transition-colors" onClick={() => {
                      setSelectedFormIds(prev => prev.includes(form.id) ? prev.filter(id => id !== form.id) : [...prev, form.id]);
                    }}>
                      <div className={`w-4 h-4 rounded border flex items-center justify-center transition-colors ${selectedFormIds.includes(form.id) ? "bg-violet-600 border-violet-600" : "border-neutral-300 dark:border-zinc-600"}`}>
                        {selectedFormIds.includes(form.id) && <Check size={12} className="text-white" strokeWidth={3} />}
                      </div>
                      <span className="text-sm font-medium text-neutral-700 dark:text-zinc-300 truncate">{form.name}</span>
                    </label>
                  ))}
                  {(!allForms || allForms.length === 0) && (
                    <div className="px-3 py-4 text-center text-xs text-neutral-400 dark:text-zinc-500">No forms found</div>
                  )}
                </motion.div>
              )}
            </AnimatePresence>
        ...
        <motion.div variants={containerVariants} initial="hidden" animate="visible" className="flex flex-col gap-4">
          ...
          <motion.div variants={itemVariants} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {statsLoading ? (
              [...Array(4)].map((_, i) => <div key={i} className="animate-pulse bg-neutral-200/50 dark:bg-zinc-800/50 rounded-xl h-24 w-full" />)
            ) : statsError ? (
              <div className="col-span-4 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md p-4 rounded-xl border border-red-200 dark:border-red-900/50 shadow-sm flex flex-col items-center justify-center h-24">
              ...
              [
                ...
                <div key={i} className="bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md p-4 rounded-xl border border-neutral-200/60 dark:border-zinc-800/60 shadow-sm flex flex-col transition-all hover:shadow-md">
                  ...
                  <div className="flex items-end justify-between mt-auto pt-2">
                    <h3 className={`text-3xl font-black tracking-tight ${stat.value === 0 || stat.value === "0%" ? "text-neutral-300 dark:text-zinc-700" : "text-neutral-800 dark:text-zinc-100"}`}>
                      {stat.value}
                    </h3>
                  </div>
                </div>
          ...
          <motion.div variants={itemVariants} className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div className="bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md p-4 rounded-xl border border-neutral-200/60 dark:border-zinc-800/60 shadow-sm flex flex-col h-[300px]">
              ...
              {deviceLoading ? (
                <div className="animate-pulse bg-neutral-200/50 dark:bg-zinc-800/50 rounded-xl h-[200px] w-full mt-4" />
              ) : deviceError ? (
                      ...
                      return (
                        <div key={i} className="flex items-center justify-between text-sm">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-medium text-neutral-600 dark:text-zinc-400">{d.name}</span>
                          </div>
          ...
          <motion.div variants={itemVariants} className="bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md p-4 rounded-xl border border-neutral-200/60 dark:border-zinc-800/60 shadow-sm flex flex-col w-full overflow-hidden">
            <p className="text-[11px] uppercase tracking-wider font-bold text-neutral-400 dark:text-zinc-500 mb-6">Weekly Activity Heatmap</p>
            {heatmapLoading ? (
              <div className="animate-pulse bg-neutral-200/50 dark:bg-zinc-800/50 rounded-xl h-[120px] w-full" />
            ) : heatmapError ? (
              ...
              <>
                <div className="overflow-x-auto pb-4">
                  <div className="flex gap-1 min-w-max">
                    <div className="flex flex-col gap-[3px] pr-2 text-[10px] text-neutral-400 dark:text-zinc-500 font-medium">
                ...
                <div className="flex items-center justify-end gap-2 text-xs text-neutral-500 dark:text-zinc-500 font-medium mt-auto pt-2">
                  <span>Less</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <motion.div variants={itemVariants} className="bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md p-4 rounded-xl border border-neutral-200/60 dark:border-zinc-800/60 shadow-sm flex flex-col">
              <p className="text-[11px] uppercase tracking-wider font-bold text-neutral-400 dark:text-zinc-500 mb-6">Traffic Sources</p>
              {trafficLoading ? (
                <div className="animate-pulse bg-neutral-200/50 dark:bg-zinc-800/50 rounded-xl h-[150px] w-full" />
              ) : trafficError ? (
                  ...
                  {trafficData.map((item, i) => (
                    <div key={i} className="flex flex-col gap-1.5">
                      <div className="flex items-center justify-between text-sm">
                        <span className="font-semibold text-neutral-600 dark:text-zinc-400">{item.source}</span>
                        <span className="font-bold text-neutral-800 dark:text-zinc-100">{item.percentage}%</span>
                      </div>
                      <div className="w-full h-2 bg-neutral-200/60 dark:bg-zinc-800/60 rounded-full overflow-hidden">
                        <motion.div 
                          initial={{ width: 0 }}
                          animate={{ width: `${item.percentage}%` }}
                          transition={{ duration: 1, ease: "easeOut" }}
                          className="h-full bg-neutral-500 dark:bg-zinc-400 rounded-full"
                        />
                      </div>
                    </div>
                  ))}
            ...
            <motion.div variants={itemVariants} className="bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md p-4 rounded-xl border border-neutral-200/60 dark:border-zinc-800/60 shadow-sm flex flex-col">
              <div className="flex items-center justify-between mb-6">
                <p className="text-[11px] uppercase tracking-wider font-bold text-neutral-400 dark:text-zinc-500">Time Distribution</p>
                {!ctLoading && !ctError && (
                  <div className="bg-violet-50 dark:bg-violet-900/20 border border-violet-100 dark:border-violet-900/50 px-3 py-1 rounded-full">
                    <p className="text-[10px] font-bold text-violet-700 dark:text-violet-400 tracking-wide uppercase">Avg: {globalStats?.avgCompletionTime || 0}s</p>
                  </div>
                )}
              </div>
              {ctLoading ? (
          ...
          <motion.div variants={itemVariants} className="bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md p-4 rounded-xl border border-neutral-200/60 dark:border-zinc-800/60 shadow-sm flex flex-col w-full overflow-hidden">
            <p className="text-[11px] uppercase tracking-wider font-bold text-neutral-400 dark:text-zinc-500 mb-6 flex items-center gap-2"><Globe size={14} /> Global Responses</p>
            {mapLoading ? (
              <div className="animate-pulse bg-neutral-200/50 dark:bg-zinc-800/50 rounded-xl h-[300px] w-full" />
            ) : mapError ? (
